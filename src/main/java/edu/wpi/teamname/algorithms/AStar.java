...
public class AStar {
		...
		while (nodesYetToSearch.size() != 0) {
			...
			for (Node nodeToSearch : dataBase.getNeighbors(currentNode.node)) {
				...
				if (!visitedNodes.containsKey(nodeToSearch)) {
					int weight = calculateWeight(nodeToSearch, end);
	...
	private int calculateWeight(Node start, Node target){
		int weight = Math.sqrt(Math.pow((start.getXCoord() - target.getXCoord),2), Math.pow((start.getYCoord - target.getYCoord),1));
		return weight;
	}
	...
	class HeuristicNode implements Comparator<HeuristicNode> {
		...
		public HeuristicNode(Node node, int weight) {
			this.node = node;
			this.weight = weight;
		}
